        ...
        <div class="center">
          ...
          >
            ...
            <div class="conter_father">
              ...
              <!-- end left -->
              <div v-if="item.auditStatus == 1" class="center-right">
                <a :href="'tel:' + item.phone" @click.stop="stopRouter()">
                  <img src="../../static/images/yitongguo_phone.png" alt="">
                </a>
                <div style="color:#72C35F;">
                  已通过
                </div>
              </div>
              <div v-if="item.auditStatus == 2" class="center-right">
                <a :href="'tel:' + item.phone" @click.stop="stopRouter()">
                  <img src="../../static/images/butongguo_phone.png" alt="">
                </a>
                <div style="color:red;">
                  未通过
                </div>
            ...
            <!-- <div style="margin-top:0.5rem">
              <span style="color:#B2B2B2">{{ item.createTime | dateFormat }}</span>
              <span v-if="item.auditStatus == 2" style="color:red;float:right;margin-right:10px">未通过</span>
              <span v-if="item.auditStatus == 1" style="color:#72C35F;float:right;margin-right:10px">已通过</span>
            </div> -->
          </div>
        </div>
      </van-list>
...
export default {
  ...
  methods: {
    getlist1 () {
      ...
      httpPost('api/shihua-server/user/findUserFinishList', data).then((res) => {
        if (res.code === 0) {
          // 加载状态结束
          this.loading = false
    ...
    routerClick (item) {
      this.$router.push({ path: '/particulars',
        query: { data: item, num: 1, active: 2 }
      })
    }
  }
}
